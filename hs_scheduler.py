...
SESSION_NUM = 6
sign_ups = 'data/Summer Session 1 HS Sign-Ups (Responses) - Form Responses 1.csv'
...
student_email = "Student Email (where we should send the Zoom link to)"
cancellations = []
...
big_class = ['AP Computer Science - David, Saturday 4-5pm PT', 'Intro to Economics - Seetal, Monday 4-5pm PT']
...
while ongoing:
    ...
    for row in data:
        ...
        for choice_idx, choice in enumerate(choices):
            ...
            if ((classname not in small_class and len(assignments[classname]) < MAX_SIZE) or (classname in small_class and len(assignments[classname]) < 13)) and classname not in row['schedule']:
                ...
                break
            elif classname not in row['waitlist'] and classname not in row['schedule'] and ((classname in small_class and len(waitlists[classname]) < 6) or (classname not in small_class and (len(waitlists[classname]) < MAX_WAITLIST_SIZE or (classname in big_class and len(waitlists[classname]) < max_big_class_waitlist)))):
...
for classname, roster in assignments.items():
    ...
    print(classname_short + " size:" + str(len(roster) + len(waitlists[classname])))
    dataframes.append(curr_dict)
...
directory = 'output/s{}/HS_SIZE{}_WAITLIST{}/'.format(SESSION_NUM, MAX_SIZE, MAX_WAITLIST_SIZE)
try:
    os.mkdir(directory)
except:
...
for data in dataframes:
    path = 'output/s{}/HS_SIZE{}_WAITLIST{}/{}.xlsx'.format(SESSION_NUM, MAX_SIZE, MAX_WAITLIST_SIZE, data['class name'])
