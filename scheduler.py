...
MAX_SIZE = 10
MAX_WAITLIST_SIZE = 5
sign_ups = 'data/S4 HS Sign-Ups (Responses) - Form Responses 1.csv'
...
cancellations = []
print_cols = ['Parent Name', 'Parent Email', 'Student Full Name', student_email, 'Student Grade', likely, 'Student Phone Number', 'Parent Phone Number']
roster_cols = ['Student Full Name', student_email, 'Student Grade', 'Student Phone Number', 'class1', 'status1', 'class2', 'status2', 'class3', 'status3', 'class4', 'status4', 'class5', 'status5']
big_class = 'Intro to Algebra 1,  Lauren, T/TH 11 AM-12 PM PDT'
...
student_emails = set()
deduped = []
curr_dict = {'name': "removed_duplicates"}

# classes = pd.read_csv(class_list, index_col = 0).to_dict('index')
# for index, dict in classes.items():
#     dict['full'] = False
#     dict['size'] = 0
...
while ongoing:
    ...
    for row in data:
        ...
        for choice_idx, choice in enumerate(choices):
            ...
            if (len(assignments[classname]) < MAX_SIZE or (classname == big_class and len(assignments[classname]) < 5 * MAX_SIZE)) and classname not in row['schedule']:
                ...
                break
            elif classname not in row['waitlist'] and classname not in row['schedule'] and len(waitlists[classname]) < MAX_WAITLIST_SIZE:
...
for classname, roster in assignments.items():
    ...
    curr_dict['name'] = classname_short
    curr_dict['status'] = []
...
curr_dict['name'] = 'full_enrollment'
dropped = 0
for student in data:
...
for data in dataframes:
    with pd.ExcelWriter('output/' + data['name'] + '.xlsx') as writer:
