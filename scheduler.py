...
PREVIOUS_INPUTS_FILE = open("prev_inputs.log","a+")

USER_INPUTS = {}
USER_INPUTS['MS_OR_HS'] = input('Middle or High School? (MS, HS): ')
PREVIOUS_INPUTS_FILE.write(USER_INPUTS['MS_OR_HS'] + '\n')
USER_INPUTS['SESSION_NUM'] = input('Session number: ')
PREVIOUS_INPUTS_FILE.write(USER_INPUTS['SESSION_NUM'] + '\n')
USER_INPUTS['FILE_NAME'] = None
while not USER_INPUTS['FILE_NAME']:
    try:
        USER_INPUTS['FILE_NAME'] = input('Path to input CSV file: (input/Summer Session 1 HS Sign-Ups (Responses) - Form Responses 1.csv): ')
        data = pd.read_csv(USER_INPUTS['FILE_NAME']).to_dict('records')
    except FileNotFoundError:
        print('ERROR: Make sure your file is a CSV and is in the location specified.\n')
        USER_INPUTS['FILE_NAME'] = None
        continue
    if not USER_INPUTS['FILE_NAME'].endswith('.csv'):
        print('ERROR: Please specify a CSV file.\n')
        USER_INPUTS['FILE_NAME'] = None
COLUMN_TITLES = data[0].keys()
input_choices = []
for key in COLUMN_TITLES:
    input_choices.append(key)
MAX_SIZE = int(input('Max class size: (Integer) '))
MAX_WAITLIST_SIZE = int(input('Max waitlist size: (Integer) '))
...
NUM_CLASS_CHOICES = int(input('How many choices did the students get? (Integer) '))
choices = []
for i in range(1, NUM_CLASS_CHOICES + 1):
    terminal_menu = TerminalMenu(input_choices, title='Choose the column for Choice {} (Press \'/\' to search)'.format(i))
    choice_index = terminal_menu.show()
    choices.append(input_choices.pop(choice_index))

terminal_menu = TerminalMenu(input_choices, title='Choose the likely question (Press \'/\' to search)'.format(i))
choice_index = terminal_menu.show()
likely = input_choices.pop(choice_index)
...
#class matching
while input('Run the sorting algo? (y or n): ') != 'n':
    assignments = {}
    waitlists = {}
    ongoing = True
    current_round = 0
...
directory = 'output/s{}/{}_SIZE{}_WAITLIST{}/'.format(SESSION_NUM, MS_OR_HS, MAX_SIZE, MAX_WAITLIST_SIZE)
try:
    os.mkdir(directory)
except:
    print('overwrote existing directory' + output_dir)
for data in dataframes:
    path = 'output/s{}/{}_SIZE{}_WAITLIST{}/{}.xlsx'.format(SESSION_NUM, MS_OR_HS, MAX_SIZE, MAX_WAITLIST_SIZE, data['class name'])
